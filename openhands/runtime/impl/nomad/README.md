...
```bash
# 1. Nomad 分配动态端口
${NOMAD_PORT_action_server} → 例如: 32768

# 2. 环境变量设置
port=${NOMAD_PORT_action_server}

# 3. 启动命令插值
/bin/bash -c "python -m openhands.runtime.action_execution_server ${port} ..."

# 4. 实际执行
python -m openhands.runtime.action_execution_server 32768 ...
```
